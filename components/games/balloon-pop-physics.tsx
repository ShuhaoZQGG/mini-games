...
interface BalloonPopPhysicsGameProps {
  levelConfig: {
    difficulty: number
    speed: number
    targetScore: number
...
function BalloonPopPhysicsGame({ levelConfig, onScore }: BalloonPopPhysicsGameProps) {
        ...
        <CardContent className="p-4">
          ...
          <div className="bg-gradient-to-b from-background to-muted rounded-lg p-8 min-h-[400px] flex items-center justify-center">
            <div className="text-center">
              <h2 className="text-2xl font-bold mb-4">Balloon Pop Physics</h2>
              <p className="text-muted-foreground mb-8">Air pressure and wind physics</p>
...
export default function BalloonPopPhysics() {
  ...
  return (
    <GameWithLevels
      gameId="balloon-pop-physics"
      gameName="Balloon Pop Physics"
      levels={levels}
      renderGame={(config, onScore) => (
        <BalloonPopPhysicsGame levelConfig={config} onScore={onScore} />
      )}
      getStars={getStars}
    />
  )
