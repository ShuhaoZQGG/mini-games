...
interface SequenceBuilderGameProps {
  levelConfig: {
    difficulty: number
    speed: number
    targetScore: number
...
function SequenceBuilderGame({ levelConfig, onScore }: SequenceBuilderGameProps) {
        ...
        <CardContent className="p-4">
          ...
          <div className="bg-gradient-to-b from-background to-muted rounded-lg p-8 min-h-[400px] flex items-center justify-center">
            <div className="text-center">
              <h2 className="text-2xl font-bold mb-4">Sequence Builder</h2>
              <p className="text-muted-foreground mb-8">Complex pattern memorization</p>
...
export default function SequenceBuilder() {
  ...
  return (
    <GameWithLevels
      ...
      gameName="Sequence Builder"
      levels={levels}
      renderGame={(config, onScore) => (
        <SequenceBuilderGame levelConfig={config} onScore={onScore} />
      )}
      getStars={getStars}
    />
  )
