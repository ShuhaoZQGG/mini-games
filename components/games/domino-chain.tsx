...
interface DominoChainGameProps {
  levelConfig: {
    difficulty: number
    speed: number
    targetScore: number
...
function DominoChainGame({ levelConfig, onScore }: DominoChainGameProps) {
        ...
        <CardContent className="p-4">
          ...
          <div className="bg-gradient-to-b from-background to-muted rounded-lg p-8 min-h-[400px] flex items-center justify-center">
            <div className="text-center">
              <h2 className="text-2xl font-bold mb-4">Domino Chain</h2>
              <p className="text-muted-foreground mb-8">Create chain reactions with physics</p>
...
export default function DominoChain() {
  ...
  return (
    <GameWithLevels
      gameId="domino-chain"
      gameName="Domino Chain"
      levels={levels}
      renderGame={(config, onScore) => (
        <DominoChainGame levelConfig={config} onScore={onScore} />
      )}
      getStars={getStars}
    />
  )
