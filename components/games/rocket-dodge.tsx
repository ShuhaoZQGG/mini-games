...
interface RocketDodgeGameProps {
  levelConfig: {
    difficulty: number
    speed: number
    targetScore: number
...
function RocketDodgeGame({ levelConfig, onScore }: RocketDodgeGameProps) {
        ...
        <CardContent className="p-4">
          ...
          <div className="bg-gradient-to-b from-background to-muted rounded-lg p-8 min-h-[400px] flex items-center justify-center">
            <div className="text-center">
              <h2 className="text-2xl font-bold mb-4">Rocket Dodge</h2>
              <p className="text-muted-foreground mb-8">Space debris avoidance with upgrades</p>
...
export default function RocketDodge() {
  ...
  return (
    <GameWithLevels
      gameId="rocket-dodge"
      gameName="Rocket Dodge"
      levels={levels}
      renderGame={(config, onScore) => (
        <RocketDodgeGame levelConfig={config} onScore={onScore} />
      )}
      getStars={getStars}
    />
  )
