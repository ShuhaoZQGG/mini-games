...
interface RhythmRunnerGameProps {
  levelConfig: {
    difficulty: number
    speed: number
    targetScore: number
...
function RhythmRunnerGame({ levelConfig, onScore }: RhythmRunnerGameProps) {
        ...
        <CardContent className="p-4">
          ...
          <div className="bg-gradient-to-b from-background to-muted rounded-lg p-8 min-h-[400px] flex items-center justify-center">
            <div className="text-center">
              <h2 className="text-2xl font-bold mb-4">Rhythm Runner</h2>
              <p className="text-muted-foreground mb-8">Platformer synchronized to music beats</p>
...
export default function RhythmRunner() {
  ...
  return (
    <GameWithLevels
      gameId="rhythm-runner"
      gameName="Rhythm Runner"
      levels={levels}
      renderGame={(config, onScore) => (
        <RhythmRunnerGame levelConfig={config} onScore={onScore} />
      )}
      getStars={getStars}
    />
  )
