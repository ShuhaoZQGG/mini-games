...
interface GravityWellGameProps {
  levelConfig: {
    difficulty: number
    speed: number
    targetScore: number
...
function GravityWellGame({ levelConfig, onScore }: GravityWellGameProps) {
        ...
        <CardContent className="p-4">
          ...
          <div className="bg-gradient-to-b from-background to-muted rounded-lg p-8 min-h-[400px] flex items-center justify-center">
            <div className="text-center">
              <h2 className="text-2xl font-bold mb-4">Gravity Well</h2>
              <p className="text-muted-foreground mb-8">Manipulate gravity to guide objects</p>
...
export default function GravityWell() {
  ...
  return (
    <GameWithLevels
      gameId="gravity-well"
      gameName="Gravity Well"
      levels={levels}
      renderGame={(config, onScore) => (
        <GravityWellGame levelConfig={config} onScore={onScore} />
      )}
      getStars={getStars}
    />
  )
