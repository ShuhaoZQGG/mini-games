...
interface DrumMachineGameProps {
  levelConfig: {
    difficulty: number
    speed: number
    targetScore: number
...
function DrumMachineGame({ levelConfig, onScore }: DrumMachineGameProps) {
        ...
        <CardContent className="p-4">
          ...
          <div className="bg-gradient-to-b from-background to-muted rounded-lg p-8 min-h-[400px] flex items-center justify-center">
            <div className="text-center">
              <h2 className="text-2xl font-bold mb-4">Drum Machine</h2>
              <p className="text-muted-foreground mb-8">Create beats with virtual drums</p>
...
export default function DrumMachine() {
  ...
  return (
    <GameWithLevels
      gameId="drum-machine"
      gameName="Drum Machine"
      levels={levels}
      renderGame={(config, onScore) => (
        <DrumMachineGame levelConfig={config} onScore={onScore} />
      )}
      getStars={getStars}
    />
  )
