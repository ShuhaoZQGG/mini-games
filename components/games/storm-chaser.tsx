...
interface StormChaserGameProps {
  levelConfig: {
    difficulty: number
    speed: number
    targetScore: number
...
function StormChaserGame({ levelConfig, onScore }: StormChaserGameProps) {
        ...
        <CardContent className="p-4">
          ...
          <div className="bg-gradient-to-b from-background to-muted rounded-lg p-8 min-h-[400px] flex items-center justify-center">
            <div className="text-center">
              <h2 className="text-2xl font-bold mb-4">Storm Chaser</h2>
              <p className="text-muted-foreground mb-8">Weather navigation and timing</p>
...
export default function StormChaser() {
  ...
  return (
    <GameWithLevels
      gameId="storm-chaser"
      gameName="Storm Chaser"
      levels={levels}
      renderGame={(config, onScore) => (
        <StormChaserGame levelConfig={config} onScore={onScore} />
      )}
      getStars={getStars}
    />
  )
