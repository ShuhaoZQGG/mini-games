...
export default function SpaceInvadersGame() {
  ...
  const invaderSpeedRef = useRef(1)
  ...
  const initializeInvaders = useCallback((waveNumber: number) => {
    ...
    const rows = Math.min(5, 3 + Math.floor(waveNumber / 2))
    const cols = 11
    ...
    invaderSpeedRef.current = 1 + (waveNumber - 1) * 0.2
    return newInvaders
  }, [])
  ...
  const moveInvaders = useCallback(() => {
    ...
    setInvaders(current => 
      current.map(invader => ({
        ...invader,
        x: invader.x + (invaderDirectionRef.current * invaderSpeedRef.current),
  ...
  const fireInvaderBullet = useCallback(() => {
    ...
    setInvaderBullets(current => [
      ...current.filter(b => b.active),
      {
        ...
        active: true,
        velocity: 3 + invaderSpeedRef.current * 0.5
      }
    ])
  }, [invaders])
  ...
  }, [playerBullet, invaderBullets, playerX, ufo])
  ...
  const checkWaveComplete = useCallback(() => {
    ...
    if (aliveInvaders.length === 0 && gameState === 'playing') {
      ...
      setTimeout(() => {
        ...
        setInvaderBullets([])
        setPlayerBullet({ x: 0, y: 0, active: false, velocity: -5 })
        setGameState('playing')
      }, 2000)
    }
  }, [invaders, gameState, wave, initializeInvaders])
  ...
  }, [gameState, moveInvaders, fireInvaderBullet, updateBullets, checkCollisions, checkWaveComplete, spawnUFO, moveUFO])
  ...
  }, [gameState, playerBullet.active, playerX])
