...
interface FingerDanceGameProps {
  levelConfig: {
    difficulty: number
    speed: number
    targetScore: number
...
function FingerDanceGame({ levelConfig, onScore }: FingerDanceGameProps) {
        ...
        <CardContent className="p-4">
          ...
          <div className="bg-gradient-to-b from-background to-muted rounded-lg p-8 min-h-[400px] flex items-center justify-center">
            <div className="text-center">
              <h2 className="text-2xl font-bold mb-4">Finger Dance</h2>
              <p className="text-muted-foreground mb-8">Multi-touch coordination game</p>
...
export default function FingerDance() {
  ...
  return (
    <GameWithLevels
      gameId="finger-dance"
      gameName="Finger Dance"
      levels={levels}
      renderGame={(config, onScore) => (
        <FingerDanceGame levelConfig={config} onScore={onScore} />
      )}
      getStars={getStars}
    />
  )
