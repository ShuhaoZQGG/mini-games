...
interface CatapultChallengeGameProps {
  levelConfig: {
    difficulty: number
    speed: number
    targetScore: number
...
function CatapultChallengeGame({ levelConfig, onScore }: CatapultChallengeGameProps) {
        ...
        <CardContent className="p-4">
          ...
          <div className="bg-gradient-to-b from-background to-muted rounded-lg p-8 min-h-[400px] flex items-center justify-center">
            <div className="text-center">
              <h2 className="text-2xl font-bold mb-4">Catapult Challenge</h2>
              <p className="text-muted-foreground mb-8">Projectile physics with trajectory</p>
...
export default function CatapultChallenge() {
  ...
  return (
    <GameWithLevels
      gameId="catapult-challenge"
      gameName="Catapult Challenge"
      levels={levels}
      renderGame={(config, onScore) => (
        <CatapultChallengeGame levelConfig={config} onScore={onScore} />
      )}
      getStars={getStars}
    />
  )
