...
interface PendulumSwingGameProps {
  levelConfig: {
    difficulty: number
    speed: number
    targetScore: number
...
function PendulumSwingGame({ levelConfig, onScore }: PendulumSwingGameProps) {
        ...
        <CardContent className="p-4">
          ...
          <div className="bg-gradient-to-b from-background to-muted rounded-lg p-8 min-h-[400px] flex items-center justify-center">
            <div className="text-center">
              <h2 className="text-2xl font-bold mb-4">Pendulum Swing</h2>
              <p className="text-muted-foreground mb-8">Physics-based swinging mechanics</p>
...
export default function PendulumSwing() {
  ...
  return (
    <GameWithLevels
      ...
      gameName="Pendulum Swing"
      levels={levels}
      renderGame={(config, onScore) => (
        <PendulumSwingGame levelConfig={config} onScore={onScore} />
      )}
      getStars={getStars}
    />
  )
